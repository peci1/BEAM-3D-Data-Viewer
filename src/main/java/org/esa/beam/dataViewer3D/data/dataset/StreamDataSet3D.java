        ...
        StreamDataSet<DataPoint3D<NumericType<X>, NumericType<Y>, NumericType<Z>>> implements DataSet3D<X, Y, Z>
{
    ...
    /**
     * Create a 3D stream data set from the given set of sources. If <code>maxPoints != null</code>, then it defines the
    ...
    public static <X extends Number, Y extends Number, Z extends Number> StreamDataSet3D<X, Y, Z> createFromDataSources(
            Integer maxPoints, DataSourceSet3D<X, Y, Z> dataSourceSet, ProgressMonitor progressMonitor)
    {
        return createFromDataSources(maxPoints, dataSourceSet.getXSource(), dataSourceSet.getYSource(),
                dataSourceSet.getZSource(), progressMonitor);
    }
    ...
    protected static class Builder3D<X extends Number, Y extends Number, Z extends Number>
    {
        ...
        public StreamDataSet3D<X, Y, Z> getResult()
        {
            ...
            while (xIt.hasNext() && yIt.hasNext() && zIt.hasNext()) {
                ...
                int hash = pointFactory.getHashCode(x, y, z);
    ...
    public static class PointFactory<X extends Number, Y extends Number, Z extends Number>
    {
         ...
         */
        public int getHashCode(NumericType<X> x, NumericType<Y> y, NumericType<Z> z)
        {
            return SimpleDataPoint3D.hashCode(x, y, z);
        }
    }
